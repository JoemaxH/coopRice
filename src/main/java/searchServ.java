...
public class searchServ extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String local = doc.getElementsByTag("local").text();
            String online = doc.getElementsByTag("online").text();
            ...
            try {
                ...
                String tmp = readUrl(urlString);
                int tmpL = (tmp.length()-1);
                String jsonS = tmp.substring(1, tmpL);
                JSONObject jsonO = new JSONObject(jsonS); 
                
                normalPrice = jsonO.getString("normalPrice");
                salePrice = jsonO.getString("salePrice");
                savings = jsonO.getString("savings");
                metacriticScore = jsonO.getString("metacriticScore");
